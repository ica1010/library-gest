...
if($('#time-slot').length>0){$('#time-slot').on('click',function(){$(".timeslot-sec").show();$(".timepicker-sec").hide();});$('#time-picker').on('click',function(){$(".timepicker-sec").show();$(".timeslot-sec").hide();});}
$(document).ready(function(){$(".next_btn").on('click',function(){$(this).closest('fieldset').next().fadeIn('slow');$(this).closest('fieldset').css({'display':'none'});$('#progressbar .active').removeClass('active').addClass('activated').next().addClass('active');});});$(".addservice-info").on('click','.trash',function(){$(this).closest('.service-cont').remove();return false;});$(".add-extra").on('click',function(){var servicecontent='<div class="row service-cont">'+
'<div class="col-md-4">'+
'<div class="form-set">'+
...
'</div>'+
'</div>'+
'</div>';$(".addservice-info").append(servicecontent);return false;});$(".hours-info").on('click','.trash',function(){$(this).closest('.hours-cont').remove();return false;});$(".add-hours").on('click',function(){var hourscontent='<div class="row hours-cont">'+
'<div class="col-md-4">'+
'<div class="form-set">'+
...
'</div>'+
'</div>'+
'</div>';$(this).parent().parent().find(".day-info").append(daycontent);$('.timepicker').datetimepicker({format:'HH:mm A',icons:{up:"fas fa-angle-up",down:"fas fa-angle-down",next:'fas fa-angle-right',previous:'fas fa-angle-left'}});return false;});if($('[data-bs-toggle="tooltip"]').length>0){var tooltipTriggerList=[].slice.call(document.querySelectorAll('[data-bs-toggle="tooltip"]'))
...
setTimeout(function(){},300);return false;});if($('.win-maximize').length>0){$('.win-maximize').on('click',function(e){if(!document.fullscreenElement){document.documentElement.requestFullscreen();}else{if(document.exitFullscreen){document.exitFullscreen();}}})}
$(document).on('mouseover',function(e){e.stopPropagation();if($('body').hasClass('mini-sidebar')&&$('#toggle_btn').is(':visible')){var targ=$(e.target).closest('.sidebar').length;if(targ){$('body').addClass('expand-menu');$('.subdrop + ul').slideDown();}else{$('body').removeClass('expand-menu');$('.subdrop + ul').slideUp();}
return false;}
$(document).on('click','#loginSubmit',function(){$("#adminSignIn").submit();});});if(document.getElementById("myRange")!=null){var slider=document.getElementById("myRange");var output=document.getElementById("currencys");output.innerHTML=slider.value;slider.oninput=function(){output.innerHTML=this.value;}}
...
$(document).on("click",".addlinks",function(){var experiencecontent='<div class="form-set links-cont">'+
...
'</div>'+
'</div>'+
'</div>';$(".settings-form").append(experiencecontent);return false;});$(".settings-form").on('click','.delete_review_comment',function(){$(this).closest('.links-cont').remove();return false;});$(document).on("click",".addnew",function(){var experiencecontent='<div class="form-set links-conts">'+
'<div class="row align-items-center">'+
'<div class="col-lg-3 col-12">'+
'<input type="text" class="form-control" placeholder="Label">'+
'</div>'+
...
'</div>'+
'</div>'+
'</div>';$(".settingset").append(experiencecontent);return false;});$(".settingset").on('click','.delete_review_comment',function(){$(this).closest('.links-conts').remove();return false;});$(document).on("click",".addlinknew",function(){var experiencecontent='<div class="form-set links-cont">'+
'<div class="row align-items-center">'+
'<div class="col-lg-3 col-12">'+
'<input type="text" class="form-control" placeholder="Label">'+
'</div>'+
...
'</div>'+
'</div>'+
'</div>';$(".settings-forms").append(experiencecontent);return false;});$(".settings-forms").on('click','.delete_review_comment',function(){$(this).closest('.links-cont').remove();return false;});$(document).on("click",".addsocail",function(){var experiencecontent='<div class="form-set countset">'+
'<div class="row align-items-center">'+
...
'</a>'+
'</div>'+
'</div> ';$(".faq").append(experiencecontent);return false;});$(".faq").on('click','.delete_review_comment',function(){$(this).closest('.counts-list').remove();return false;});$(".settings-form").on('click','.trash',function(){$(this).closest('.links-cont').remove();return false;});$(document).on("click",".add-links",function(){var experiencecontent='<div class="row form-row links-cont">'+
...
'</td>'+
'</tr>';$(".add-table-items").append(experiencecontent);return false;});$(document).on("click",".addnewheader",function(){var experiencecontent='<div class="col-12">'+
...
'</div>'+
'</div>'+
'</div>';$(".add-headers").append(experiencecontent);return false;});$(document).on("click",".delete-links",function(){$(this).parent().parent().parent().parent().hide();});if($('.datatable').length>0){$('.datatable').DataTable({"bFilter":false,"sDom":'fBtlpi','pagingType':'numbers',"ordering":true,"language":{search:' ',sLengthMenu:'_MENU_',searchPlaceholder:"Search...",info:"_START_ - _END_ of _TOTAL_ items",},initComplete:(settings,json)=>{$('.dataTables_filter').appendTo('#tableSearch');$('.dataTables_filter').appendTo('.search-input');},});}
...
if($('.toggle-passworda').length>0){$(document).on('click','.toggle-passworda',function(){$(this).toggleClass("fa-eye fa-eye-slash");var input=$(".pass-inputa	");if(input.attr("type")=="password"){input.attr("type","text");}else{input.attr("type","password");}});}
...
$("#imgInp").change(function(){readURL(this);});if($('#phone').length>0){var input=document.querySelector("#phone");window.intlTelInput(input,{utilsScript:"assets/plugins/intltelinput/js/utils.js",});}
...
$('.theme-image').on('click',function(){$('.theme-image').removeClass('active');$(this).addClass('active');});$('.transfer-lists').on('click',function(){$('.transfer-lists').removeClass('active');$(this).addClass('active');});$('.themecolorset').on('click',function(){$('.themecolorset').removeClass('active');$(this).addClass('active');});$('.counters').each(function(){var $this=$(this),countTo=$this.attr('data-count');$({countNum:$this.text()}).animate({countNum:countTo},{duration:3000,easing:'linear',step:function(){$this.text(Math.floor(this.countNum));},complete:function(){$this.text(this.countNum);}});});if($('.owl-carousel.gallery-slider').length>0){$('.owl-carousel.gallery-slider').owlCarousel({loop:true,margin:24,nav:true,smartSpeed:2000,navText:["<i class='fa-solid fa-angle-left'></i>","<i class='fa-solid fa-angle-right'></i>"],dots:false,navContainer:'.mynav3',responsive:{0:{items:1},550:{items:2},700:{items:2},1000:{items:3}}})}
setTimeout(function(){$('#loader');setTimeout(function(){$("#loader").fadeOut("slow");$("#overlayer").delay(2000).fadeOut("slow");},100);},500);if($('.sidebar .active').length>0){var container=$('div'),scrollTo=$('.sidebar .active');container.animate({scrollTop:scrollTo.offset().top-container.offset().top+container.scrollTop()-100});}
$(".social-info").on('click','.trash',function(){$(this).closest('.social-cont').remove();return false;});$(".social-add").on('click',function(){var socialcontent='<div class="row align-items-center social-cont">'+
...
'</div>'+
'</div>'+
'</div>';$(this).parent().find(".social-info").append(socialcontent);$('.select').select2({minimumResultsForSearch:-1,width:'100%'});return false;});$('.bank-box').on('click',function(){$('.bank-box').removeClass('active');$(this).addClass('active');});$('.loc-set').on('click',function(){$('.loc-set').removeClass('soc-active');$(this).addClass('soc-active');});var chatAppTarget=$('.chat-window');(function(){if($(window).width()>991)
...
return false;});})();if($(window).width()<=1199){if($('#task_chat').length>0){$(document).on('click','#task_chat',function(){$('.sidebar-overlay').toggleClass('opened');$('#task_window').addClass('opened');return false;});}}
if($(window).width()>767){if($('.theiaStickySidebar').length>0){$('.theiaStickySidebar').theiaStickySidebar({additionalMarginTop:125});}}})(jQuery);